...
spa.chat = (function (){
  ...
  // 引数：
  //  * position_type ー enum('closed','opened',または'hidden')
  //  * callback ― アニメーションの最後のオプションのコールバック。
  // このコールバックは単一引数としてスライダーdivを表すjQueyコレクションを受け取る。
  // 動作：
  // このメソッドはスライダーを要求された位置に移動する。
  // 要求された位置が現在の位置の場合には、なにもせずにtrueを返す。
  // 戻り値：
  //  * true ー 要求された位置に移動した
  //  * false ー 要求された位置に移動していない
  // 例外発行：なし
  ...
  onListchange = function (event) {
    ...
    if (!list_html) {
      list_html = String()
        ...
        + 'To chat alone is the fate of all gerat souls...<br><br>'
        + 'No one is online'
        + '</div>';
      clearChat();
    }
  ...
  // 用例：spa.chat.configModule({ slider_opened_em: 18});
  // 目的：初期化前にモジュールを構成する
  // 引数：
  //  * set_chat_anchor ー オープンまたはクローズ状態を示すように
  // URIアンカーを変更するコールバック。このコールバックは要求された状態を
  // 満たせない場合にはfalseを返さなければいけない。
  //  * chat_model ー インスタントメッセージングと
  //    やり取りするメソッドを提供するチャットモデルオブジェクト。
  //  * people_model ー モデルを保持する人々のリストを管理する
  //    メソッドを提供するピープルモデルオブジェクト。
  //  * slider_*構成。すべてはオプションのスカラー。完全なリストはmapConfig.settable_mapを参照。
  //    用例：slider_opened_emはem単位のオープン時の高さ
  // 動作：
